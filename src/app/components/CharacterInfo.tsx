...
import { MovieRating } from './CharacterRating';
...
export const CharacterInfo = ({
  ...
  <div className='absolute z-10 w-full rounded-b-md bg-zinc-800 p-2 shadow-md transition lg:p-4'>
    ...
    <div className='mt-4 flex flex-col gap-2 text-sm'>
      <div className='flex flex-row items-center gap-2'>
        <InformationCircleIcon className='w-4' /> {character.house} house
        <InformationCircleIcon className='w-4' />{' '}
        {character.ancestry === '' ? '' : character.ancestry} ancestry
      </div>
    </div>
    <div className='mt-4 flex flex-row items-center gap-2 text-[8px] text-white lg:text-sm'></div>
    <div className='flex flex-row items-center gap-2'>
      <UserGroupIcon className='w-4' />{' '}
      <MovieRating rating={character.rating} />
    </div>
  </div>
);
